...
atriaModel <- function(connectionDetails,
  ...
  ParallelLogger::logInfo('Extracting Atria settings')
  # create covariate settings
  covariateInfo <- getCovariateInfo('atria-bleed')
  ...
  plpModel <- tryCatch({list(model = getModel(model = 'atria-bleed'),
                   analysisId = paste0('Analysis_',analysisId),
                   hyperParamSearch = NULL,
                   index = NULL,
                   trainCVAuc = NULL,
                   modelSettings = list(model = 'atria-bleed', modelParameters = NULL),
                   metaData = NULL,
                   populationSettings = attr(population, "metaData"),
                   trainingTime = NULL,
                   varImp = getModel(model = 'atria-bleed'),
                   dense = T,
                   cohortId = cohortId,
                   outcomeId = outcomeId,
                   endDay = endDay,
                   covariateMap = NULL,
                   predict = getPredict(getModel(model = 'atria-bleed'), mapping = map)
  )},
  ...
  result$model$modelName <- 'atria-bleed'
